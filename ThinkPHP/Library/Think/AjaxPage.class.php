...
class AjaxPage {
    ...
    protected $coolPages   ;
    // 分页显示定制
    protected $config  = array('header'=>'<span class="dataPage_number">%listRows%条/页-共%totalRow%条 当前第%nowPage%页 / 共%totalPage%页 </span>','prev'=>'上一页','next'=>'下一页','first'=>'第一页','last'=>'最后一页','theme'=>' %header% %upPage% %downPage% %first%  %prePage%  %linkPage%  %nextPage% %end%');
    // 默认分页变量名
    protected $varPage;
    ...
    public function show() {
        ...
        $pageStr  =  str_replace(
            array('%header%','%nowPage%','%totalRow%','%listRows%','%totalPage%','%upPage%','%downPage%','%first%','%prePage%','%linkPage%','%nextPage%','%end%'),
            array($this->config['header'],$this->nowPage,$this->totalRows,$this->listRows,$this->totalPages,$upPage,$downPage,$theFirst,$prePage,$linkPage,$nextPage,$theEnd),$this->config['theme']);
        return $pageStr;
    }
