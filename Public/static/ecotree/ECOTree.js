...
ECONode = function (id, pid, dsc, w, h, c, bc, target, meta,heads) {
	this.id = id;
	this.pid = pid;
	this.dsc = dsc;
	this.w = w;
	...
	for (var k = 0; k < this.nodeChildren.length; k++)
	{
		...
		switch(tree.render)
		{
			case "CANVAS":
				...
				// 	tree.ctx.strokeStyle = tree.config.linkColor;
				// }else{
				// 	tree.ctx.strokeStyle = "#e6e6e6";
				// 	// console.log(tree.ctx.strokeStyle);
				// }
				// console.log(tree.ctx.strokeStyle);
				
				tree.ctx.beginPath();		
				switch (tree.config.linkType)//M
				{
					case "M":						
...
ECOTree = function (obj, elm) {
	this.config = {
		...
		nodeBorderColor : "fff",
		nodeSelColor : "#FFFFCC",
		levelColors : ["#5D5FC3","#E5648D","#31BDBC","#678DCE","#E5648D","#4d5bb2"],
		...
		defaultTarget : '',
		collapsedImage : '/Public/jg/img/plus.gif',
		transImage : '/Public/jg/img/trans.gif',
		expandedImage:"",
		canvasPaddingLeft:800,
		canvasWidth:2000,
		XPosition:1600
	}
...
//CANVAS functions...
//方框线性图
ECOTree._roundedRect = function (ctx,x,y,width,height,radius,node) {
	ctx.beginPath();
	if(node.dsc){
...
ECOTree._firstWalk = function (tree, node, level) {
        ...
        node.XPosition = tree.config.XPosition;
        node.YPosition = 0;
        node.prelim = 0;
        node.modifier = 0;
		...
		if (!node._isAncestorCollapsed())
		{
			switch (this.render)
			{
				case "CANVAS":
					...
					ECOTree._roundedRect(this.ctx,node.XPosition,node.YPosition,node.w,node.h,5,node);
					this.ctx.restore();
					
					//HTML part...
					s.push('<div id="' + node.id + '" class="econode" style="top:'+(node.YPosition+this.canvasoffsetTop)+'px; left:'+(node.XPosition+this.canvasoffsetLeft+this.config.canvasPaddingLeft)+'px; width:'+(node.w-10)+'px; height:'+node.h+'px;line-height:'+node.h+'px;" ');
					...
					{
						//id, pid, dsc, w, h, c, bc, target, meta,heads
						s.push('<div id="t' + node.id + '"   style="color:'+node.c+';width:100%; height:'+node.h+'px;cursor: pointer;" onclick="gourl(\''+node.target+'\')" >');
						// s.push(node.dsc);
						var mgtop = node.h/20;
						s.push('<div style="line-height:20px;text-align:center;margin-top:'+mgtop+'px;word-wrap:break-word;">'+node.dsc+'</div>');
						if(node.heads){
							s.push('<div style="line-height:20px;text-align:center;margin-top:20px;word-wrap:break-word;">('+node.heads+')</div>');
						}
						s.push('</div>');
					}				
					else
...
ECOTree.prototype.toString = function () {	
	...
	{
		case "CANVAS":
			s.push('<canvas id="ECOTreecanvas" width="'+this.config.canvasWidth+'px" height="2000px" class="cans" style="padding-left:'+this.config.canvasPaddingLeft+'px"></canvas>');
			break;
...
ECOTree.prototype.add = function (id, pid, dsc, w, h, c, bc, target, meta,heads) {	
	...
	var heads_ = heads || this.config.heads;
	...
	var node = new ECONode(id, pid, dsc, nw, nh, color, border, tg, metadata,heads);	//New node creation...
